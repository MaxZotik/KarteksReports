...
{
    public class RepositoryArchivePointExamined : IRepository<ArchivePoint>
    {
        public List<ArchivePoint> ArrayObjects { get; set; }

        private readonly Logger _logger;
        private readonly IConnectDb _connectionString;
        private readonly RepositoryDiagramExamined _repositoryDiagram;
        private readonly RepositoryTestData _repositoryTestData;
        private readonly RepositoryChannelConfig _repositoryChannelConfig;

        public RepositoryArchivePointExamined(IConnectDb connectDb, RepositoryDiagramExamined repositoryDiagramExamined, RepositoryTestData repositoryTestData, RepositoryChannelConfig repositoryChannelConfig) 
        {
            ArrayObjects = new();
            _logger = new();
            _connectionString = connectDb;
            _repositoryDiagram = repositoryDiagramExamined;
        ...
        private float GetDataArchive(int gearboxID, int mvk, int channel, int parameter, int frequency, DateTime? dateTimeStart, DateTime? dateTimeEnd)
        {
            ...
            else
            {
                ...
                {
                    _ = _logger.LogAsync(@$"RepositoryArchivePointExamined : {ex.Message}");
